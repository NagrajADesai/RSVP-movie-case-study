...
SELECT COUNT(*) AS No_of_Rows_movie FROM movie;
-- '7997'
...
SELECT 
    ...
    SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS year_sum,
...
FROM movie m 
    ...
    ON m.id = rm.movie_id
        ...
        ON rm.name_id = n.id
			INNER JOIN ratings r 
            ON r.movie_id = m.id
WHERE m.languages LIKE '%hindi%' AND rm.category = 'actress'
